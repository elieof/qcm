...
public final class TokenAuthenticationFilter extends GenericFilterBean {
    ...
    private static final String HEADER_TOKEN_VALIDITY = "X-Auth-Token-Validity";
    ...
    public TokenAuthenticationFilter(AuthenticationService authenticationService, String logoutLink, String tokenLink) {
	...
	if (canRequestProcessingContinue(httpRequest) && httpRequest.getMethod().equals("POST")) {
	    ...
	    if (authenticated) {
		//refresh token when posting to endpoint "/token" ans user authenticate
	...
	String tokenInfo = authenticationService.authenticate(username, password);
	if (tokenInfo != null) {
	    httpResponse.setHeader(HEADER_TOKEN, tokenInfo);
	    httpResponse.setHeader(HEADER_TOKEN_VALIDITY, authenticationService.getTokenValidity());
	    // TODO set other token information possible: IP, ...
	} else {
	    String unauthorizedError = new ApiErrorMessage(
		    new ApiAuthenticationException("invalid username or password"))
	    .jsonValue();
	    httpResponse.getWriter().write(unauthorizedError.toString());
	...
	if (currentLink(httpRequest).equals(tokenLink)) {
	    String token = httpRequest.getHeader(HEADER_TOKEN);
	    // we go here only authenticated, token must not be null
	    String tokenInfo = authenticationService.refreshToken(token);
		if (tokenInfo != null) {
		    httpResponse.setHeader(HEADER_TOKEN, tokenInfo);
		    httpResponse.setHeader(HEADER_TOKEN_VALIDITY, authenticationService.getTokenValidity());
		    // TODO set other token information possible: IP, ...
